...
fn infer_grn_whole_runs_on_synthetic_gem() {
    let dir = std::env::temp_dir().join(format!("celloracle_grn_int_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    write_minimal_mouse_grn_parquet(&dir).unwrap();

    let var_names = vec!["Reg1".to_string(), "Tgt1".to_string(), "Reg2".to_string()];
    let net =
        GeneNetwork::new("mouse", &var_names, Some(dir.to_str().unwrap())).expect("GeneNetwork");
