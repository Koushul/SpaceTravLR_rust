...
Release: [v1.1.0](https://github.com/Koushul/SpaceTravLR_rust/releases/tag/v1.3.0). For `main`, swap `refs/tags/v1.1.0` for `refs/heads/main`.
...
3. **GRN parquet files** — training needs `mouse_network.parquet` and/or `human_network.parquet` (inferred from gene naming). Resolution order: `[grn].network_data_dir` in `spaceship_config.toml`, then the **`SPACETRAVLR_DATA_DIR`** environment variable (directory containing those files), then the executable’s `data/` or `../data/`, then walking up from the process current directory looking for `data/`, then `./data/` under cwd. Prebuilt release binaries do **not** embed a repo path; copy the `data/` parquet files from the repo or set `SPACETRAVLR_DATA_DIR`. You do **not** need to run the CLI from the repository root if one of these finds the files.
