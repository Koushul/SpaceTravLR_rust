...
2. **Compute backend** — the **`spacetravlr`** binary prefers **WGPU** when [wgpu](https://github.com/gfx-rs/wgpu) can acquire an adapter. If none is available, it uses Burn’s **NdArray (CPU)** backend. Set `SPACETRAVLR_FORCE_CPU=1` (or `true`) to force CPU. You can use the same behavior with `SPACETRAVLR_DISABLE_WGPU=1` if you want to avoid the GPU stack entirely.

   **WGPU / CubeCL panic (“plane instructions … not supported by the hardware”):** Burn’s WGPU backend uses CubeCL, which may autotune to subgroup (“plane”) kernels. Some Linux GPU drivers or headless adapters advertise WGPU but fail at runtime with a panic inside `cubecl-runtime` autotune. **Fix:** run training with CPU only, e.g. `SPACETRAVLR_FORCE_CPU=1 spacetravlr …` (slower but stable). Updating the GPU driver or trying a different Vulkan ICD can also help if you need GPU speed.
