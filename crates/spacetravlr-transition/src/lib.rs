...
/// How to combine signal and null transition matrices after `P - P_null`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum NullSubtractMode {
    /// SpaceTravLR default: keep positive mass only, then row-renormalize.
    #[default]
    ClipRenorm,
    /// SpaceOracle / cartography.py: keep signed `P - P_null` (no clip, no renorm).
    Raw,
}

/// Subtract null-model P (computed from zero delta).
        ...
        .map(|(i, row)| {
            ...
            match mode {
                NullSubtractMode::Raw => {
                    ...
                    for (&j, &pn) in &null_map {
                        if !seen.contains(&j) && pn != 0.0 {
                            out.push((j, -pn));
                        }
                    }
                    out
                }
                NullSubtractMode::ClipRenorm => {
                    let mut out: Vec<(usize, f64)> = Vec::new();
                    let mut sum = 0.0_f64;
                    for (j, ps) in row {
                        let pn = null_map.get(&j).copied().unwrap_or(0.0);
            ...
            match mode {
                NullSubtractMode::Raw => {
                    for (j, oj) in o_row.iter_mut().enumerate() {
                        ...
                        *oj = sig_flat[base + j] - null_flat[base + j];
                    }
                }
                NullSubtractMode::ClipRenorm => {
                    let mut sum = 0.0_f64;
...
pub struct TransitionUmapParams {
    ...
    /// How `P - P_null` is applied. Use [`NullSubtractMode::Raw`] for SpaceOracle parity.
    pub null_subtract_mode: NullSubtractMode,
}
...
pub fn compute_umap_transition_grid(
    ...
    let p = if params.use_full_graph && n <= params.full_graph_max_cells {
        ...
        if params.remove_null {
            ...
            p_dense = subtract_null_dense(&p_dense, &p0, params.null_subtract_mode);
        }
        UmapTransitionP {
            p_sparse: vec![],
        ...
        if params.remove_null {
            ...
            p_sparse = subtract_null_sparse(p_sparse, &p0, params.null_subtract_mode);
        }
        UmapTransitionP {
            p_sparse,
            p_dense_opt: None,
